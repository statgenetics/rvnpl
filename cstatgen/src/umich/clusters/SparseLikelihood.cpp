...
//         trees[i].freqs.Set(1.0 / trees[i].freqs.dim);
      ...
      while (true)
		 ...
		 std::cout<<"Next\n"<<std::endl;
		 std::cout<<fabs(llk1+llk0)<<" "<<tol<<" "<<ztol<<std::endl;
         double precision = max(fabs(llk1 + llk0) * tol, ztol);
		 std::cout<<fabs(llk1-llk0)<<" "<<precision<<std::endl;
	  ...
	  std::cout<<"treeCount:"<<treeCount<<"newTreeCount:"<<newTreeCount<<std::endl;
...
double SparseLikelihood::SetLikelihood(HaplotypeSet * set)
   {
   std::cout<<"Entering SetLikelihood ..."<<std::endl;
   ...
   set->scale = 0;
   std::cout<<"likelihood-0"<<"set-graphCount:"<<set->graphCount<<std::endl;
   for (int i = 0; i < set->graphCount; i++)
      ...
      std::cout<<"likelihood-1, i="<<i<<std::endl;

      GraphLikelihood(graph);
      std::cout<<"likelihood-2"<<std::endl;
      ...
      std::cout<<"likelihood-3"<<std::endl;
      }
...
double SparseLikelihood::GraphLikelihood(HaplotypeGraph * graph)
   {
   std::cout<<"  Entering GraphLikelihood ..."<<std::endl;
   if (!secondPass) factors->Zero();
   std::cout<<"GraphLikelihood-0"<<std::endl;
   ...
   std::cout<<"GraphLikelihood-2"<<std::endl;

   std::cout<<"  Leaving GraphLikelihood ..."<<std::endl;
...
double SparseLikelihood::GraphLikelihood(HaplotypeGraph * graph, int tree, int i)
   {
   Unknown * u;
   //std::cout<<"tree:"<<tree<<" i:"<<i<<std::endl;
   while (true)
      {
      if (i == graph->unknowns.Length())
	  {
		 //std::cout<<"before CurrentLikelihood, tree:"<<tree<<" i:"<<i<<std::endl;
