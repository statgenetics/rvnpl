...
class RData(dict):
    def __init__(self, samples_vcf, tfam):
        ...
        self.samples_vcf = range(len(samples_vcf))
...
class RegionExtractor:
	...
	if varIdx == 0:
	    ...
	    if self.include_vars_file is not None:
...
class MarkerMaker:
            ...
            with env.lock:
                if not env.prephased:
                    ...
                    with stdoutRedirect(to = tmp_log_output + '.log'):
                        ...
                        #haplotypes[item] = self.haplotyper.Execute(data.chrom, varnames[item],sorted(positions), data.getFamSamples(item),self.rsq,tmp_log_output)[0]
                else:
	...
	if tmp_mafs != {}:
        ...
        #
        if self.maf_cutoff is not None or self.single_markers:
            if self.freq_by_fam:
...
def main(args):
    ...
    if not args.vanilla and cache.check():
        ...
        try:
            ...
            if args.freq_by_fam is not None:
            ...
            if env.commonvar_counter.value:
                env.log('{:,d} variants ignored due to having MAF > {} and specified requirement'.\
