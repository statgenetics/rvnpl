...
#Modified on npl_all_combined_v08_test2_tmp7_tmp4_large7_s4_tmp4_fast2.py
#Do not shuffle founder genotypes when doing permutations, only randomly assign offspring genotypes
#Include parent-offspring pairs
...
import numpy as np
import ibd_module                        
import cmissingparents5 as cmissingparents
import cmissing_infer6
import cconv2
import sall_module
import cpostInv3
import time
...
#################
#parse arguments
parser = argparse.ArgumentParser(description = "NPL scoring functions to detect over sharing of alleles in affecteds in extended families")
...
parser.add_argument('--output',metavar='PATH',help = "Path for output files")
parser.add_argument('--exact',action='store_true',help="get the exact distribution of Z-score and calculate p value from it")
parser.add_argument('--info_only',action='store_true',help="include only informative families")
parser.add_argument('--perfect',action='store_true',help="use perfect data approximation in calculating Z-score")
parser.add_argument('--kc',action='store_true',help="use Kong&Cox(1997) extension for analytical p-values")
parser.add_argument('--sall',action='store_true',help="enable calculation of NPL-all")
parser.add_argument('--rvibd',action='store_true',help="calculate IBD for RV only")
parser.add_argument('--n_jobs',metavar='N',type=int,help = 'number of multiprocess')
parser.add_argument('--cut','-c',metavar='FLOAT',type=float,help = 'threshold for adaptive permutations')
parser.add_argument('--fam_rep',metavar='N',default=50000,type=int,help = 'times of permutations for each family')
parser.add_argument('--rep',metavar='N',default=1000000,type=int,help = 'times of permutations')
parser.add_argument('--force',action='store_true',help = 'keep permutation times unchanged')
parser.add_argument('--perfect_max',metavar='N',default=500000,type=int,help = 'maximum for inheritance vector iterations')
    ...
    while True:
		...
		if allele_key not in all_ibd.keys() or sall_flag and all_ibd[allele_key][1]==0:
			...
			#print repr(procID)+" new_alleles:"+repr(new_alleles)
		    	...
		    	#print repr(procID)+" GT_infer end"
                    	#print "new conditional_prob"+repr(len(conditional_prob['~combined'][0].keys()))
...
class Family:
	...
	def core_struct(self,nf):
		...
		if self.fam_dict[nf]['offspring']:
		    ...
		    if len(mates)==1:
			founder_gt.append(sorted(self.fam_dict[mates[0]]['gt'][2*fam.mid:2*fam.mid+2]))
			...
			for mate in mates:
			    mate_branch=[]
			    mate_fgt=[sorted(self.fam_dict[mate]['gt'][2*fam.mid:2*fam.mid+2])]
		...
		if not self.missing_all:
			...
			if not info_flag:
                ...
                if len(set(allgt)) == 1 or not self.missing_all and not info_flag:
                    self.info = False
		if set(allgt)==set([0,1]) and args.rvibd:
		    self.wt_fam = True
...
#classify affecteds and missing genotype imputation
	...
	def remove(self,iid):
	    ...
	    #print "return_gt:"+repr(len(return_gt))
	    ...
	    tmp_dic,miss_gt_id=cmissing_infer6.apply(self.mp_freq,self.invnf,self.founder,self.parents,self.mates,self.offspring,self.sex,sorted_miss_persons,alleles,infer_flag)
	    cut_size=50
	    cut_prob=1
	...
	def assign_allele(self,v):
        ...
        def sall(self,v,fam_allele,fmarker):
   	    ...
   	    try:
		self.affnf[0]+1
            	s_all = sall_module.apply(self.affected,fam_allele,self.fallele,fmarker,args.rvibd)
	    except TypeError:
		...
		if fmarker==[]:
            	    s_all = sall_module.apply(tmp_aff,tmp_fam_allele,self.fallele,fmarker,False)
		else:
            	    s_all = sall_module.apply(tmp_aff,tmp_fam_allele,self.fallele,fmarker,args.rvibd)
            return s_all
	...
	def getGT(self,v,foundergt):
        ...
        def postInvtmp(self,tmpalleles={}):
            #calculate the posterior inheritance vectors and their corresponding probability
            if tmpalleles == {}:
                for iid in self.fam_dict.keys():
                    tmpalleles[iid] = copy.copy(self.fam_dict[iid]['gt'][2*tmpm:2*tmpm+2])
            tmpgt=[]
            for iid in tmpalleles.keys():
                tmpgt+=tmpalleles[iid]
            postinv=[]
            if len(set(tmpgt))==1:
                #uninformative
                afnfnum=len(self.invnf)
                for i in xrange(pow(2,2*afnfnum)):
                    tmp = bin(i)[2:]                     #get binary format
                    inv = '0'*(2*afnfnum-len(tmp))+tmp  #make tmp complete with bit number
                    postinv.append(inv)
            else:
                tmpv = {}
                v = {}
                ...
                order = []
                branch_order = []
                for nf in self.invnf:    #for each nonfounder in inheritance vector
                    print 'nf:'+repr(nf)
                    fid,mid = self.fam_dict[nf]['parents']
                    pgeno = tmpalleles[fid]+tmpalleles[mid]  #parents genotype
                    tmpv[nf]=[] #tmp inheritance vector
                    tmpos=[]
                    pos[nf]=[]
                    for allele in tmpalleles[nf]:
                    ...
                    if self.fam_dict[fid]['parents'] == ['0','0'] and self.fam_dict[mid]['parents'] == ['0','0']:           #if the nonfounder is the 2nd generation
                        order+=branch_order
                        v[nf] = []
                        offspring=0
                        branch_order=[nf]
                        for element in pos[nf]:
                            ...
                            if inh not in tmpv[nf]:
                                tmpv[nf][-1].append(inh)
                        for t_v in tmpv[nf]:
                            v[nf].extend(t_v)
			for tnf in v.keys():
			    print tnf, len(v[tnf])
                    else:                       #if it is 3rd generation or beyond nonfounder
                        offspring +=1
                        branch_order.append(nf)
                        branch_id = branch_order[0]
                        if self.fam_dict[fid]['parents'] != ['0','0']:  #father important
                            order_idx=branch_order.index(fid)
                            for element in pos[nf]:
                                ...
                                for idx,fpos in enumerate(pos[fid]):
                                    if fpos[pa] < 2:             #if paternal allele is from grandpa
                                        ...
                                        tmpinv=[1,ma%2]
				    uniq_tmpv=[]
                                    for t_tmpv in tmpv[fid][idx]:
                                        tmpv[nf][-1].append(t_tmpv+tmpinv)   #parental inv and self inv
                                        if t_tmpv[-2:] not in uniq_tmpv:
                                            uniq_tmpv.append(t_tmpv[-2:])
				    for t_tmpv in uniq_tmpv:                                        
					for vi in [tv for tv in v[branch_id] if len(tv)==2*offspring]:
                                            ances_v=vi[2*order_idx:2*order_idx+2]
                                            if ances_v == t_tmpv:
                                                v[branch_id].append(vi+tmpinv)
                            for mv in [vi for vi in v[branch_id] if len(vi)<2*offspring+2]:    #remove redundant vectors
                                v[branch_id].remove(mv)
                            #print v
                        else:                                      #if mother important
                            order_idx=branch_order.index(mid)
                            for element in pos[nf]:
                                pa, ma = sorted(element)[:]
                                tmpv[nf].append([])
                                for idx,mpos in enumerate(pos[mid]):
                                    if mpos[ma-2] < 2:             #if maternal allele is from grandpa
                                        tmpinv=[pa%2,0]
                                    elif mpos[ma-2] > 1:
                                        tmpinv=[pa%2,1]
				    uniq_tmpv=[]
                                    for t_tmpv in tmpv[mid][idx]:
                                        tmpv[nf][-1].append(t_tmpv+tmpinv)
				    	if t_tmpv[-2:] not in uniq_tmpv:
					    uniq_tmpv.append(t_tmpv[-2:])
				    for t_tmpv in uniq_tmpv:
                                        for vi in [tv for tv in v[branch_id] if len(tv)==2*offspring]:
                                            ...
                                            if ances_v == t_tmpv:
                                                v[branch_id].append(vi+tmpinv)
                            for mv in [vi for vi in v[branch_id] if len(vi)<2*offspring+2]:
                                v[branch_id].remove(mv)
                        print "v: "+repr(len(v[branch_id]))+" "+repr(v)
                order+=branch_order
                #print order
                #print tmpv
                #print v
                ##the postinv is in the order of 2nd generation nonfounders
                ##followed by their corresponding offsprings
                trans_v={}
                for key in v.keys():
                    trans_v[key]=[''.join(map(str,tmplist)) for tmplist in v[key]]
                postinv = self.conv(trans_v)
                #print postinv
                ###get the postinv in the order of self.invnf
                if order != self.invnf:
                    ordered_postinv = []
                    for pinv in postinv:
                        tmppost = ''
                        for nf in self.invnf:
                            idx=order.index(nf)
                            tmppost += pinv[2*idx:2*idx+2]
                        ordered_postinv.append(tmppost)
                    postinv = ordered_postinv
            return postinv
	...
	def cal_sall(self,pinv,fmarker,pinv_sall=[],pinv_key_dict={},tmp_alleles={},t_alleles={},rvibd=False):
		...
		for tmp_gt,prob in tmp_dic.iteritems():
			...
			for aid,bid in self.sib:
			    ...
			    ibd = ibd_module.sib_ibd(genotype,rv_flag=args.rvibd)
			    total_ibd+=ibd
			for aid,bid in self.cousin:
			    ...
			    ibd = ibd_module.cousin_ibd(genotype,rv_flag=args.rvibd)
			    total_ibd+=ibd
			for aid,bid in self.hsib:
			    ...
			    ibd=ibd_module.hsib_ibd(genotype,rv_flag=args.rvibd)
			    total_ibd+=ibd
			for aid,bid in self.un:      #(uncle,nephew)
			    ...
			    ibd = ibd_module.un_ibd(genotype,rv_flag=args.rvibd)
			    total_ibd+=ibd
			for aid,bid in self.gp:      #(grandpa,grandson)
			    ...
			    genotype = tmp_alleles[aid]+tmp_alleles[grand_other]+\
				    tmp_alleles[bp[0]]+tmp_alleles[bp[1]]+tmp_alleles[bid]
			    ibd = ibd_module.gp_ibd(genotype,rv_flag=args.rvibd)
			    total_ibd+=ibd
			if sall_flag:
			    ###NPL-all
			    #get posterior inheritance vectors for probable allele configurations
			    postinv=cpostInv3.apply(self.invnf, self.founder, self.parents, tmp_alleles, args.rvibd)
			    len_postinv=len(postinv)
			    #print fmarker
			    #print "postinv:"+repr(len_postinv)+" pinv_key_dict:"+repr(len(pinv_key_dict.keys()))
			    postinv_list=set(postinv) if args.rvibd else postinv
			    for pinv in postinv_list:
				t_alleles=self.assign_allele(pinv)
				t_sall=self.cal_sall(pinv,fmarker,pinv_sall,pinv_key_dict,tmp_alleles,t_alleles,rvibd=args.rvibd)
				if args.rvibd:
			...
			for pinv in postinv_list:
				...
				if t_alleles=={}:
				    t_alleles=self.assign_allele(pinv)
				t_sall=self.cal_sall(pinv,fmarker,pinv_sall,pinv_key_dict,tmp_alleles,t_alleles,args.rvibd)
				if args.rvibd:
				    total_sall+=t_sall*postinv.count(pinv)
				else:
				    total_sall+=t_sall
	...
	def nullibd(self,rep,sall_flag=False,infer_flag=2,simple=False):                            
	...
	def null_perfect(self, update_null=True):
	#null distribution of IBDs under perfect data
	    ...
	    print rep
	    if rep>args.perfect_max:
		inv_list=random.sample(xrange(rep),args.perfect_max)
	    else:
		inv_list=range(rep)
	    self.pinv_sall = []
	    chunk_size=len(inv_list)//args.n_jobs if len(inv_list)>=args.n_jobs else 1
	...
	def null_perfect_rvibd(self,sall_flag=False):
	#null distribution of IBDs under perfect data
	    ...
	    combined_dist={}
            prob_and_mean=[]
	    if self.simple:
	        local_perfect_max=int(args.perfect_max/len(foundergt_dic.keys()))
	    else:
		local_perfect_max=min(int(args.perfect_max/len(foundergt_dic.keys())),5)
...
def randomsample(dic,rep):
    ...
    result = p.imap(ransample,repeat((structnum,fam_struct),rep), chunksize=rep//args.n_jobs) 
    #result.wait()
    p.close()
    p.join()
...
def K_C(coeff,min_b):
	...
	if len(delta)==0:
	    ...
	    if len(delta)>1:
		    ...
		    print "delta:"+repr(tmp_delta)+" coeff:"+repr(coeff)
                    try:
...
already_calculated=[]
gene_path='/mnt/nfs/linhai/data/nuclearDichotom/zongxiao_simulation/NuclearFamily/whole-genome/DAR_OR1_300nuclear_3sib_allmiss/SEQL_removemiss_median_ExAC/tmp_genes'
#gene_path='/mnt/nfs/linhai/data/ADSP_vcf/vcf_snv_rezipped/vcf_snv/gnomAD_combined_annotated/whole_exome_NFE_Dutch_filtered_AN_DP_GQ_BOTHpass_missrateByCenter_allvcfvar_gnomAD_rvhaplo_chr4_wt4/tmp_genes'
'''
with open(gene_path) as gfh:
    for line in gfh:
        already_calculated.append(line.strip())
'''
for (dirpath,dirname,filename) in os.walk(args.path):
        ...
        if len(fped) == 1 and len(fdat) == 1:
            ...
            if genename in already_calculated:
                continue
		...
		if None in fam_to_analyze[markername[m]]:
		    ...
		    print "fid:"+repr(fid)
                    ...
                    if args.snv and maf_by_marker != {}:
                        ...
                        if fid in CHP_freq_byfam[markername[m]]:
			...
			#print fam.ibd_total, fam.ibd_sall
		    except:
			fam.err=True
		    if fam.err:
			...
			if args.rvibd:
				...
				if fam_combined_info not in fam_conditional_prob_perfect:
				    fam.null_perfect_rvibd(sall_flag=args.sall)
			...
			if fam.missing_all!=[] and not fam.wt_fam:
				...
				fam.nullibd(rep=50,sall_flag=args.sall)
			    ...
			    #print fam.ibd_total,sorted(fam.null_ibd)[-10:]
                            ...
                            if fam.famstruct not in famstruct:
                                fam.nullibd(rep=100,sall_flag=args.sall)
	    ...
	    small_num=3
	    small_cutoff = 1E-11 if args.snv else 1E-9
	    min_asymp_pv = min([x for x in asymp_pv if x !=1])[0]
	    if min_asymp_pv >=args.cut or min_asymp_pv<small_cutoff:
		small_num=1
		...
		if fam_num==0:
		    pr_s, pr_sall = 0.5, 0.5
                if apv[0]>=args.cut and apv[1]>=args.cut or apv[0]<small_cutoff and apv[1]<small_cutoff:
		    ...
		    rtext+='permutations:{}\n'.format(rep)
			    ...
			    if not increase_permutation:
			    	fam.null_ibd = []
			    #print fid, sorted([(k,v) for k,v in fam.conditional_prob['~combined'][0].iteritems()], key=lambda x: x[1], reverse=True)
			    #print fam.conditional_prob['~combined'][1]
			    if not fam.simple:
				infer_full_flag=2
				...
				fam.GT_infer(infer_flag=infer_full_flag)
                    		fam.ibd_total,fam.ibd_sall=fam.cal_ibd(sall_flag=args.sall)
				#fam.all_ibd={}
			    #print sorted([(k,v) for k,v in fam.conditional_prob['~combined'][0].iteritems()], key=lambda x: x[1], reverse=True)
			    #print fam.conditional_prob['~combined'][1]
                            if fam.missing_all!=[]:
				fam_combined_info=(fam.famstruct,fam.conditional_prob['~combined'][0])
				if args.perfect and not args.rvibd:
				    if fam_combined_info not in fam_conditional_prob:
                                    	fam.nullibd(rep=fam_rep,sall_flag=pall_flag,infer_flag=infer_full_flag,simple=True)
					fam_conditional_prob.append(fam_combined_info)
					...
					try:
					    ...
					    fam.null_perfect_rvibd(sall_flag=pall_flag)
					    fam_conditional_prob_perfect.append(fam_combined_info)
					    fam_conditional_prob_null_perfect[str(fam_conditional_prob_perfect.index(fam_combined_info))]=[fam.dist_s,fam.null_mean,fam.null_std,fam.sall_null_mean,fam.sall_null_std]
					...
					fam.dist_s=None
                                        if fam_combined_info not in fam_conditional_prob:
                                            fam.nullibd(rep=fam_rep,sall_flag=pall_flag,infer_flag=infer_full_flag)
                                            fam_conditional_prob.append(fam_combined_info)
                                            fam_conditional_prob_null[str(fam_conditional_prob.index(fam_combined_info))]=[fam.null_mean,fam.null_std,fam.null_ibd,fam.sall_null_mean,fam.sall_null_std]
                                        else:
                                            fam.null_mean,fam.null_std,fam.null_ibd,fam.sall_null_mean,fam.sall_null_std=fam_conditional_prob_null[str(fam_conditional_prob.index(fam_combined_info))]
				...
				if fam.null_ibd:
					...
					print max([x[0] for x in fam.null_ibd]),fam.ibd_total
				    	if args.perfect:
				            fam.nullibd(rep=fam_rep,sall_flag=pall_flag,infer_flag=infer_full_flag,simple=True)
				    	else:
				            fam.nullibd(rep=fam_rep,sall_flag=pall_flag,infer_flag=infer_full_flag)
				...
				pfamstruct.pop('info',None)
				if pfamstruct not in famstruct_perfect:
				    fam.null_perfect(update_null=False)
				    famstruct_perfect.append(pfamstruct)
            ...
            ###################################################
                    if cz_dist == []:
                        if args.exact:              #random sample
                            ...
                            cz_dist,csall_dist=randomsample(z_dist[idx_apv],rep)
                    if args.exact:
